...
{\centering
  ...
  \faHome\ New York, NY
  \quad $|$ \quad
  \faCode\ \href{https://github.com/ajsinger1/arisinger-resume}{Resume Source}\\[6pt]
}
...
\begin{itemize}[leftmargin=*, nosep, topsep=2pt, parsep=1pt]
  \item Built a production grade LLM inference platform as primary IC alongside principal engineers (L7/L8) --- disaggregated serving with Nvidia Dynamo, KV-aware routing, and a custom orchestration layer supporting A/B deployments, fault tolerance, and rate limiting, fully IaC on Kubernetes (AWS CDK)
  ...
  \item Supported post-training and evaluation efforts --- ran SFT jobs with VeRL and debugged config issues for research scientists
\end{itemize}

\vspace{1pt}
\textit{Software Development Engineer I} \hfill \textsc{March 2024 --- June 2025}
\begin{itemize}[leftmargin=*, nosep, topsep=2pt, parsep=1pt]
  \item Implemented an asynchronous pipeline that routed 100K+ ad creatives through AI-based transformation services, converting assets into new format-compliant variants --- unlocked hundreds of millions of new DSP impressions and substantial incremental annual revenue
\end{itemize}
...
\begin{itemize}[leftmargin=*, nosep, topsep=2pt, parsep=1pt]
  \item Built a smart door handle with custom PCB for automatic unlock/locking based on user proximity + capacitive touch along with a companion app for remote access. Handled all embedded programming, cloud backend, integration with the iOS app.
\end{itemize}
...
\vspace{1pt}
\textit{Bachelor of Science in Engineering in Computer Science, Mathematics Minor} \hfill \textsc{May 2023}
\begin{itemize}[leftmargin=*, nosep, topsep=2pt, parsep=1pt]
  \item GPA: 3.95/4.00 (Summa Cum Laude)
\end{itemize}

\vspace{1pt}
\textit{Relevant Coursework:} Systems of Generative AI, Machine Learning, Artificial Intelligence, Natural Language Processing, Operating Systems, Compilers, Computer Architecture, Cryptography, Computer Networks, Web Systems, Embedded Systems, Logic Design
